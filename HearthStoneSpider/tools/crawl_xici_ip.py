...
def crawl_ips():
    ...
    for i in range(2):
        ...
        for ip_info in ip_list:
            print('insert {0}://{1}:{2} {3}'.format(ip_info[2], ip_info[0], ip_info[1], ip_info[3]))
            db.insert('proxy_ip', {'ip': ip_info[0], 'port': ip_info[1], 'proxy_type':ip_info[2], 'speed': ip_info[3]})
    ...
    def judge_ip(self, ip, port, proxy_type):
        ...
        try:
            ...
            if code >= 200 and code < 300:
                ...
                print('invalid ip and port code:', code)
                self.delete_ip(ip, port)
